...
resource "kubectl_manifest" "static_jupyter_pv" {
  count = var.enable_fsx_for_lustre ? 1 : 0
...
YAML
  ...
  depends_on = [resource.kubectl_manifest.fsx_storageclass, kubernetes_namespace_v1.jupyterhub]
}
